...
namespace ObfuscatorBase.Utils
{
    internal static class Randomization
    {
        ...
        private static string m_glitch = Obfuscator.Charset.glitchChars;
        ...
        public static string GetRandomGlitchString(int minLength, int maxLength)
        {
            StringBuilder builder = new StringBuilder(GetRandomInt(minLength, maxLength));
            for (int i = 0; i < builder.Capacity; i++)
            {
                builder.Append(m_glitch[GetRandomInt(0, m_glitch.Length)]);
            }
            string result = builder.ToString();
            builder = null;
            return result;
